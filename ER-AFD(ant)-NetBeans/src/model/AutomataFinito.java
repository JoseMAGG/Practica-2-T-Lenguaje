...
/** Clase que representa un autómata finíto sea determinístico o no, maneja todos sus estados y transiciones.
 *
 * @author Jose
 */
public class AutomataFinito {
     ...
     * método, generando un autómata finíto determinístico mínimo con base en 
     * la expresión regular
     * 
     * @param er
     * @throws Exception 
     */
    public void ERtoAFD(String er) throws Exception {
        er = er.trim();
        if (er.isEmpty()) {
            throw new Exception("No se puede transformar la ER a AF porque está vacía");
        }
        simbolosEntrada = Regex.hallarSimbolos(er);
        if (simbolosEntrada.isEmpty()) {
            throw new Exception("La expresión debe tener al menos un símbolo de entrada");
        }
     ...
     * @throws Exception 
     */
    private Estado ERtoAFND(String er) throws Exception {
        /*
        ...
        if (er.length() == 1) {
            if (Regex.esOperador(er.charAt(0))) {
                throw new Exception("La expresión no puede ser únicamente un operador");
            }
        ...
        do {
            ...
            if (split == null) {
                ...
                if (auxSubER.length() == 2) {
                    ...
                    throw new Exception("Hay errores en la ER");
                }
                switch (op) {
                    case STAR:
                    ...
                    case PLUS:
                        ...
                        break;
                    default:
                        throw new Exception("No se identificaron bien las operaciones");
                }
    ...
    /** Simplifica un autímata finito NO deterministico generado con construcciones
     * de Thompson y lo convierte en autómata finíto determinístico
     * 
     * @param inicial 
     */
    private void AFNDtoAFD(Estado inicial) {
        ...
        for (Object[] elementoLista : cierresLambda) {
    ...
    /** Agrega el estado de error al autómata finíto, en cada transición vacía 
     * de cada estado y al final del array de estados
     * 
     */
    ...
    public boolean validarCadena(String cadena) throws Exception {
        if(cadena.isEmpty()){
            throw new Exception("No se puede validar una cadena vacía");
        }
        if (estados == null) {
            throw new Exception("No se ha generado un AFD para validar la cadena");
        }
        if (!StringManager.validarSimbolosEntrada(cadena, simbolosEntrada)) {
            throw new Exception("Se han ingresado símbolos que no pertenecen a los símbolos de entrada");
        }
